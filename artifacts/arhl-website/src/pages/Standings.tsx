            ...
            <TableBody>
              {isLoading ? (
                ...
                <TableRow key={team.teamId} className="border-border/50 hover:bg-white/5 transition-colors">
                  <TableCell className="text-center font-display text-2xl text-muted-foreground">{team.rank}</TableCell>
                  <TableCell>
                    <Link href={`/teams/${team.teamId}`} className="flex items-center gap-4 group">
                      <div className="w-10 h-10 rounded-full border border-white/20 flex items-center justify-center bg-background/50 group-hover:scale-110 transition-transform" style={{ borderColor: team.primaryColor }}>
                        <span className="text-sm font-bold" style={{ color: team.primaryColor }}>{team.teamAbbreviation}</span>
                      </div>
                      <span className="font-display tracking-wide text-xl group-hover:text-primary transition-colors">{team.teamName}</span>
                    </Link>
                  </TableCell>
                  <TableCell className="text-center text-lg">{team.wins + team.losses}</TableCell>
                  ...
                  <TableCell className={`text-center font-bold text-lg ${team.goalDifferential > 0 ? 'text-green-500' : team.goalDifferential < 0 ? 'text-red-500' : 'text-muted-foreground'}`}>
                    {team.goalDifferential > 0 ? '+' : ''}{team.goalDifferential}
                  </TableCell>
                </TableRow>
              ))}
              {standings?.length === 0 && (
                <TableRow><TableCell colSpan={9} className="text-center text-muted-foreground py-12">No standings available.</TableCell></TableRow>
              )}
            </TableBody>
          </Table>
        </CardContent>
